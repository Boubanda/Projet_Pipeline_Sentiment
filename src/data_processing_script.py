...
def preprocess_data(df):
    ...
    # Supprimer les lignes où 'content' ou 'score' sont NaN
    df = df.dropna(subset=['content', 'score'])

    # Appliquer la fonction de nettoyage sur chaque texte
    df['text'] = df['content'].apply(clean_text)  # 'review' remplacé par 'content'
...
def main():
    ...
    df_processed.to_csv('/Users/utilisateur/Projet_Pipeline_Sentiment/data/processed_dataset.csv', index=False)
    print("Les données prétraitées ont été sauvegardées dans 'processed_dataset.csv'.")
